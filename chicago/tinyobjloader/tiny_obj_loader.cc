  ...
  const char* mtl_basepath)
{
  ...
  //CALCULATE NORMALS/TANGENTS/BINORMALS
  ...
  for(unsigned s = 0; s < shapes.size(); s++) {
	  ...
	  shapes[s].mesh.binormals.resize(shapes[s].mesh.positions.size());
	  ...
	  for(unsigned i = 0; i < shapes[s].mesh.indices.size() / 3; i++) {
		...
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3] * 3] += binormal.x;
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3] * 3 + 1] += binormal.y;
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3] * 3 + 2] += binormal.z;
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3 + 1] * 3] += binormal.x;
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3 + 1] * 3 + 1] += binormal.y;
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3 + 1] * 3 + 2] += binormal.z;
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3 + 2] * 3] += binormal.x;
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3 + 2] * 3 + 1] += binormal.y;
		shapes[s].mesh.binormals[shapes[s].mesh.indices[i * 3 + 2] * 3 + 2] += binormal.z;
	...
	for(unsigned v = 0; v < shapes[s].mesh.positions.size() / 3; v++) {
		...
		glm::vec3 b = glm::normalize(glm::vec3(
			shapes[s].mesh.binormals[v * 3],
			shapes[s].mesh.binormals[v * 3 + 1],
			shapes[s].mesh.binormals[v * 3 + 2]));
		shapes[s].mesh.binormals[v * 3] += b.x;
		shapes[s].mesh.binormals[v * 3 + 1] += b.y;
		shapes[s].mesh.binormals[v * 3 + 2] += b.z;
	 }
